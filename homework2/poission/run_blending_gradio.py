...
def is_point_in_polygon(point, polygon):
    """
    判断点是否在多边形内部
    :param point: 给定的点，格式为 (x, y)
    :param polygon: 多边形顶点列表，格式为 [(x1, y1), (x2, y2), ...]
    :return: True if 点在多边形内, False if 点在多边形外
    """
    x, y = point
    n = polygon.shape[0]
    inside = False
    ...
    for i in range(n):
        x1, y1 = polygon[i,:]
        x2, y2 = polygon[(i + 1) % n,:]  # 下一顶点，最后一个顶点和第一个顶点连成一条边
        
        # 判断射线是否与当前边相交
        if y > min(y1, y2):  # 点的y坐标大于边的最低点
            if y <= max(y1, y2):  # 点的y坐标小于边的最高点
                if x <= max(x1, x2):  # 点的x坐标小于边的最大点
                    if y1 != y2:  # 排除水平边
                        # 计算交点的x坐标
                        x_intersect = (y - y1) * (x2 - x1) / (y2 - y1) + x1
                    if x1 == x2 or x <= x_intersect:  # 点在交点的左边或正好在交点上
                        inside = not inside  # 每次交点切换inside的状态
...
def create_mask_from_points(points, img_h, img_w):
    ...
    # gridY = np.arange(img_h, dtype=np.int16)
    # X, Y = np.meshgrid(gridX, gridY)
    for i in range(img_h):
        for j in range(img_w):
            mask[i,j] = 255*is_point_in_polygon((i,j),points)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    weight = torch.zeros(3,3,3,3).to(device)
    for i in range(3):
        weight[i,i,:,:] = torch.tensor([[0,1,0], [1,-4,1], [0,1,0]]).to(device)
    foreground_Lap = torch.nn.functional.conv2d(foreground_img.to(device), weight, bias=None, stride=1, padding=1, dilation=1, groups=1).squeeze(0)
    blended_Lap = torch.nn.functional.conv2d(blended_img.to(device), weight, bias=None, stride=1, padding=1, dilation=1, groups=1).squeeze(0)

    foreground_Lap = foreground_Lap[foreground_mask.squeeze(0).repeat(3,1,1)>0.5]
    blended_Lap = blended_Lap[background_mask.squeeze(0).repeat(3,1,1)>0.5]
    loss = torch.sum((foreground_Lap-blended_Lap) ** 2)
    ...
    loss = loss.to(foreground_img.device)
    return loss
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    print(foreground_polygon_points)
